...
### Analysis Rationale
Before starting the analysis, the 4 weather datasets and energy dataset was merged for easier analysis. Additionally, one-hot encoding was used for `Station` so that the different cities could be added as a feature for analysis. A train-test split with 80/20 split was used as well, with the target variable being `DOM_MW`, aka the energy usage. A linear regression model and random forest model were used to predict energy usage based on the weather data. The goal with these two models was to have a simpler and then more complex prediction models to see if there were significant differences in th performance metrics. For each model, root mean squared error (RMSE), mean absolute error (MAE), and r-squared (R^2) were calculated for the performance overall, performance on normal weather conditions, and performance on the extreme weather conditions. Using these three metrics shows the error overall (MAE), the error with large errors being penalized more (RMSE), and the variance in the model (R^2). The results were combined into a pandas Dataframe and stored in a duckdb table. 

### Visualization Rationale
The visualization was created with the goal of showing the differences in the models' performances in comparison to each other and to the three different scenarios. A bar chart was chosen because it allows for a clean visualization, while still showing all the necessary information and is easy to see the comparisons. There are two bar charts in the figure because of the difference in values between RMSE and MAE, and R^2. RMSE and MAE have similar value ranges that there is no issue plotting them on the same y-axis, however R^2 has a range of 0-1, which did not show well on the same plot as the other two metrics. They are shown in two separate graphs to increase readability and ensure that correct, and no misleading conclusions are drawn. 
